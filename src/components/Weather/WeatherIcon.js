...
const WeatherIcon = ({ data }) => {
	...
	useEffect(() => {
		if (isSuccess) {
			const matchWeatherCode = weatherCodes.find((code) => code.code === weather.weather.code);
			// pod will be "d" for day, "n" for night
			setWeatherIcon(weather.pod === "n" ? "moon" : matchWeatherCode.icon ? matchWeatherCode.icon : "sun");
		}
	...
	return (
		<Wrapper>
			<Transition show={isFetching || isError} onExited={() => setShowIcon(true)}>
				<img src={smile} alt="" />
			</Transition>
