...
const Draw = () => {
	...
	useEffect(() => {
		...
		const defineCanvasSize = () => {
			const finalSize = window.innerWidth <= 768 ? wrapperRef.current.clientWidth - 50 : window.innerWidth / 3 <= 400 ? window.innerWidth / 3 : 400;
	...
	return (
		<Wrapper ref={wrapperRef}>
			<p>Draw this:</p>
			<div className="row">
				<h2>{!randomWord.isError ? randomWord?.data : "dog"} </h2>
